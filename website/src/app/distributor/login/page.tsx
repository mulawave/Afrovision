...
import { useDistributorAuth } from "@/lib/DistributorAuthContext";

export default function DistributorLoginPage() {
  const router = useRouter();
  const { login, isAuthenticated, isLoading } = useDistributorAuth();
    ...
    <main className="min-h-screen flex items-center justify-center px-6 py-24">
      ...
      <div className="relative w-full max-w-md animate-fade-in-up">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-av-white mt-6">Distributor Portal</h1>
          <p className="text-sm text-av-light-orange mt-1">Sign in to manage your distribution network</p>
        </div>
        ...
        <div className="rounded-2xl bg-av-card border border-av-input-border/30 p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="password" className="block text-xs font-semibold text-av-light-orange mb-1.5">
                Password
              </label>
